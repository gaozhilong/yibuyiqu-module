 ...
 *
 */
public class AuthServer extends Verticle {
	...
	private void login(final Message<JsonObject> message) {
	...
	private void logout(final Message<JsonObject> message) {
		JsonObject session = new JsonObject();
	...
	private void authorise(final Message<JsonObject> message) {
		JsonObject session = new JsonObject();
		session.putString("sessionID", message.body().getString("sessionID"));
